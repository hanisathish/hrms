...
class AttendanceRosterController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $this->employees = Employee::manager()
            ...
            return View::make('admin.attendances.attendance-roster-all', $this->data);
    }
...
as a'), "a.employee_id", "=", "employees.id")
    ...
    public function filterAttendanceRoster(Request $request)
    {
        ...
        foreach($employees as $employee) {
            ...
            foreach($employee->attendance as $attendance) {
                $textDay = date("D",strtotime(date('Y-m-'.Carbon::parse($attendance->date)->day)));

                $final[$employee->id.'#'.$employee->full_name][Carbon::parse($attendance->date)->day] =
                    ($textDay == "Sat" || $textDay == "Sun") ? 
                        'WO' :
                        (($attendance->status == 'absent') ?
                            'AL' :
        ...
        $view = View::make('admin.attendances.roster-load-all', $this->data)->render();
        //old $view = View::make('admin.attendances.load', $this->data)->render();
